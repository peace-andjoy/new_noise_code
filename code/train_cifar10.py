#!/usr/bin/env python
# coding: utf-8
# 控制num_filter不变 改变num_res，为了加快运行速度，分为train_all1和train_all2
# In[1]:
...
for train_sigma in range(0.03,0.55,0.005):
    noise_scale = np.sqrt(0.5)*train_sigma* 255
    ...
    for alpha_train in train_list:
        ...
        for repeat_time in range(repe):
            ...
            if alpha_train == 0:
            ...
            elif alpha_train == 5.5:
                num2 = len(alpha_trains2)
                noisy_tr = np.tile(X_train, (times_tmp,1))#50000*times_tmp,3072
                X_train = X_train.reshape(num_trainSamples,32,32,3)
                for alpha_ in alpha_trains2:
                    temp = noisy_tr.copy() #50000*times_tmp*3072
                    # noise = np.apply_along_axis(lambda x: stable_noise_row(x, alpha = alpha_, scale=noise_scale), axis=1, arr=temp)
                    noise = stable_noise_hundred(temp,alpha=alpha_,beta=0,scale = noise_scale)
                    noise = noise.reshape(num_trainSamples*times_tmp,32,32,3)
                    X_train = np.r_[X_train,noise]
                    print(np.shape(X_train))
                y_tr = np.tile(y_train, (times_tmp*num2+1,1))
                y_train = encoder.fit_transform(y_tr)
                y_train = to_categorical(y_train, num_classes)
                y_train = np.array(y_train)
                y_train = y_train.reshape(num_trainSamples*(times_tmp*num2+1),10)
            elif alpha_train == 'mix':
                X_noise = stable_noise_mixture_hundred(noisy,alphas = alpha_trains,beta=0,scale = noise_scale)
                ...
                X_noise = stable_noise_mixture_hundred(noisy,alphas = alpha_trains2,beta=0,scale = noise_scale)
                print(np.shape(X_noise))
                X_noise = X_noise.reshape(50000*times,32,32,3)
                X_train = X_train.reshape(50000,32,32,3)
                X_train = np.r_[X_train,X_noise]
            ...
            save_times = repeat_time+3
            model.save('./model/resnet_trainalpha{}_trainsigma{}_repeattimes{}_numfilters{}_numblocks{}.h5'.format(alpha_train,train_sigma,save_times,num_filter_block,num_res))
